...
const Wishlist = () => {
  ...
  const addToCart = (item, e) => {
    ...
    // Navigate to cart page with the specific item details
    navigate('/cart', { 
      state: { 
        addedItem: item,
        fromWishlist: true,
  ...
  const handleCardClick = (item) => {
    console.log('Navigating to product details:', item.name);
        ...
        {wishlistItems.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {wishlistItems.map((item) => (
              <div
                key={item.id}
                className="bg-[var(--bg-primary)] rounded-lg shadow-sm border border-[var(--border-light)] overflow-hidden hover:shadow-lg transition-all duration-300 group cursor-pointer"
